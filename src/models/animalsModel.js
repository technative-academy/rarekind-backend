...
export const getAnimalById = async (id) => {
    const [rows] = await pool.query(
        'SELECT  a.id, a.collection_id, a.name, a.classification_id, c.name AS classification_name, a.description, a.image_url FROM animals AS a JOIN classifications AS c ON a.classification_id = c.id WHERE a.id = ?',
        [id]
    )
    return rows[0]
}
